### Hi, I'm Seema Kumari 👋
**Data Analyst | Data Scientist | Analytics Engineer**

Turning data into business decisions with SQL, Python, Power BI & Microsoft Fabric.

🎓 B.Tech (Mechanical Engineering) @ NIT Agartala, India
💼 Data Analytics Intern @ Bosch Chassis Systems India
🌍 Open to Data Analyst / Data Scientist roles 

---

### 📊 Impact at a Glance
- 💰 Flagged **$9.32M revenue exposure** across **8,200+ products** from 601K+ reviews
- 🚨 Built fraud-detection model on **6M+ transactions** (94% accuracy, 0.13% fraud rate)
- 📦 Optimized supply-chain decisions across **63K+ orders**
- 🧪 Ran A/B tests — **+58% conversion lift**, statistically validated
- ☁️ Delivered end-to-end **Power BI + Microsoft Fabric** analytics solutions

---

### 🚀 Featured Projects

**🕵️ [Fraud Detection System](https://github.com/seema-kri/fraud-detection-system)**
`Python` `Scikit-learn` `Streamlit`
End-to-end fraud pipeline on 6M+ mobile-money transactions — 94% accuracy, deployed as live app.

**🛍️ [Sephora Product & Sentiment Analytics](https://github.com/seema-kri/Sephora-Product-Sentiment-Analytics)**
`SQL` `Python` `NLP` `Power BI`
Flagged $9.32M in revenue exposure across 8,216 products from 601K+ reviews.

**🚚 [Vantara — Supply Chain Intelligence Platform](https://github.com/seema-kri/Vantara-Smart-Supply-Chain-Intelligence-Platform)**
`SQL` `Python` `Power BI` `ETL`
End-to-end analytics on 63K+ orders — delivery performance, revenue trends, ops efficiency.

**🧪 [A/B Testing — Pharma App](https://github.com/seema-kri/AB-testing-pharma-app)**
`Python` `SciPy` `Experimentation`
Packaging A/B test — +58% relative conversion lift (p = 0.00027), segmented ship recommendation.

---

### 🛠️ Tech Stack
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat&logo=postgresql&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![Power BI](https://img.shields.io/badge/Power_BI-F2C811?style=flat&logo=powerbi&logoColor=black)
![Microsoft Fabric](https://img.shields.io/badge/Microsoft_Fabric-0078D4?style=flat&logo=microsoft&logoColor=white)
![Excel](https://img.shields.io/badge/Excel-217346?style=flat&logo=microsoftexcel&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)

`DAX` `ETL` `Data Modeling` `NLP` `Statistics` `A/B Testing`

---

### 🎯 What I Do
**Extract → Clean → Model → Analyze → Visualize → Recommend**
I don't just build dashboards — I find problems, quantify impact, and turn data into decisions.

---

### 📫 Connect With Me
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/seema-kumari-375763308)
[![Medium](https://img.shields.io/badge/Medium-12100E?style=for-the-badge&logo=medium&logoColor=white)](https://medium.com/@seemakri136)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/seema-kri)
